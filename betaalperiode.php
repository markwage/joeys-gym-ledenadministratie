...
if (isset($_POST['delete'])) {
	...
	if($sql_result = mysqli_query($dbconn, $sql_select)) {
	    if(mysqli_num_rows($sql_result) > 0) {
	        $aantalrecords = mysqli_num_rows($sql_result);
	        writelogrecord("betaalperiode","Er worden ".$aantalrecords." records uit betalingen verwijderd");
	        $sql_delbetalingen = mysqli_query($dbconn, "DELETE FROM betalingen WHERE betaalperiodeID = '$delID'");
	    }
...
if (isset($_POST['save']) || (isset($_POST['submit']))) {
	...
	if ((!$_POST['betaalperiode'] || $_POST['betaalperiode'] == "") && (!$formerror)) {
	    ...
	    echo("<script> alert('Er is geen betaalperiode ingevuld')</script>");
	    ...
	    if (isset($_POST['submit'])) {
	        ...
	        if($sql_result = mysqli_query($dbconn, $sql_select)) {
	            while($row_selectleden = mysqli_fetch_array($sql_result)) {
                    ...
                    VALUES ('".$lidID."', '".$betaalperiodeID."', '".$contributiebedrag."')";
...
if ($aktie == 'disp') {
	...
	while($row_betaalperiode = mysqli_fetch_array($sql_betaalperiode)) {
		...
		echo '<tr class="'.$rowcolor.'">
			...
			<td class="button"><a href="betalingen.php?aktie=betalingen&betalingenID='.$id.'"><img src="./img/buttons/icons8-euro-48.png" alt="betalingen bekijken" title="Betalingen bekijken" /></a></td>
	...
	<form name="betaalperiode" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
		...
		<table>
			...
			<tr>
				...
				<td><input type="text" name="betaalperiode" size="4" maxlength="6" value="<?php if (isset($frm_betaalperiode)) { echo $frm_betaalperiode; } ?>"> (YYYYMM)</td>
			</tr>
		</table>
		<br />
		<?php 
		if ($aktie == 'toevoegen') echo '<input class="button" type="submit" name="submit" value="submit">'; 
		if ($aktie == 'edit') echo '<input class="button" type="submit" name="save" value="save">'; 
		if ($aktie == 'delete') {
		    echo '<blockquote>Indien je deze betaalperiode delete worden ook alle betalingen verwijderd die aan deze betaalperiode gekoppeld zijn</blockquote><br /><br />';
